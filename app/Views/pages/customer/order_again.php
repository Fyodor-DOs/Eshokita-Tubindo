    ...
    <div class="row g-3 mb-3">
      <div class="col-md-3">
        ...
        <input type="text" class="form-control"
          value="<?= esc(($customer['kode_rute'] ?? '') . ' - ' . ($customer['nama_wilayah'] ?? '')) ?>" readonly>
      </div>
      <div class="col-md-3">
        ...
        <table class="table align-middle">
          ...
          <tbody>
            <?php foreach ($products as $p): ?>
              <tr data-id="<?= $p['id_product'] ?>" data-price="<?= (float) $p['customer_price'] ?>"
                data-stock="<?= (int) ($p['qty'] ?? 0) ?>">
                <td>
                ...
                <td>
                  <?php $stock = (int) ($p['qty'] ?? 0); ?>
                  <input type="number" class="form-control qty" min="0" max="<?= $stock ?>" placeholder="0" <?= $stock <= 0 ? 'disabled' : '' ?>>
...
<script>
  ...
  document.getElementById('formOrderAgain').addEventListener('submit', async (e) => {
    ...
    if (hasStockError) {
      Swal.fire({ icon: 'error', title: 'Stok Tidak Cukup', text: stockErrorMsg.trim() });
      __oaProcessing = false;
      submitBtn && (submitBtn.disabled = false);
      return;
    }
