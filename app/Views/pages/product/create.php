...
<?= view('components/Breadcrumb', ['segment1' => 'product', 'segment2' => 'create']) ?>
<div class="card">
  <div class="card-body">
        ...
        <div class="col-md-4">
          ...
          <select name="id_category" class="form-select">
            ...
            <?php foreach($categories as $c): ?>
              <option value="<?= $c['id_category'] ?>"><?= esc($c['name']) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
        <div class="col-md-3">
          <label class="form-label">Berat Satuan (kg)</label>
          <input name="unit" type="number" min="0.01" step="0.01" class="form-control" placeholder="contoh: 5" required />
        </div>
        <div class="col-md-3">
          <label class="form-label">Stok (QTY)</label>
          <input name="qty" type="number" min="0" class="form-control" value="0" required />
        </div>
        <div class="col-md-3">
